...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
	...
	<div class="body">
		<c:if test="${not empty paperPageBean.list}">
			<table class="table table-striped">
				...
				<c:forEach items="${paperPageBean.list}" var="paper">
					<!-- 情景颜色 -->
		...
		<c:if test="${empty paperPageBean.list }">
			<h2 align="center">没有试卷</h2>
		</c:if>
	</div>
	<div class="page">
		 <div class="person-bottom" style="padding-top: 10px; float: right;">
		    <button class="btn btn-default" onclick="gotoPage(${paperPageBean.previousPage})">&lt;&nbsp;上一页</button>
       		<c:forEach items="${paperPageBean.pageBar}" var="pageNum">
			    <button  class="btn btn-default" onclick="gotoPage(${pageNum})">
					<c:if test="${paperPageBean.currentPage ==  pageNum}">
						<font color="red">${paperPageBean.currentPage}</font>
					</c:if>
					<c:if test="${paperPageBean.currentPage ne  pageNum}">
						${pageNum}
					</c:if>
				</button>
			</c:forEach>
           <button class="btn btn-default" onclick="gotoPage(${paperPageBean.nextPage})">下一页&nbsp;&gt;</button>
				<span class='bottom-txt' style=''>共${paperPageBean.totalPage}页  共${paperPageBean.totalRecord}条记录</span>
				<span class="bottom-txt">到第<input type="text" value="${paperPageBean.currentPage}" id="currentPage" style="width: 40px; height: 35px; text-align: center;">页</span>
...
<script type="text/javascript">
	...
	function gotoPage(currentPage){
		...
		window.location.href="/OA/paper/findAllPaper?currentPage=" + currentPage;
	}
</script>
