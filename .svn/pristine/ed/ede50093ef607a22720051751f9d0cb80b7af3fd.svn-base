...
<div class="header">
	...
	<div class="body">
		<c:if test="${not empty examPageBeanByCondition.list}">
			<table class="table table-striped">
				...
				<c:forEach items="${examPageBeanByCondition.list}" var="exam">
					<!-- 情景颜色 -->
					<tr class="active">
		...
		<c:if test="${empty examPageBeanByCondition.list }">
			<h2 align="center">没有考试</h2>
		</c:if>
	</div>
	<div class="page">
		 <div class="person-bottom" style="padding-top: 10px; float: right;">
		    <button class="btn btn-default" onclick="gotoPage(${examPageBeanByCondition.previousPage})">&lt;&nbsp;上一页</button>
       		<c:forEach items="${examPageBeanByCondition.pageBar}" var="pageNum">
			    <button  class="btn btn-default" onclick="gotoPage(${pageNum})">
					<c:if test="${examPageBeanByCondition.currentPage ==  pageNum}">
						<font color="red">${examPageBeanByCondition.currentPage}</font>
					</c:if>
					<c:if test="${examPageBeanByCondition.currentPage ne  pageNum}">
						${pageNum}
					</c:if>
				</button>
			</c:forEach>
           <button class="btn btn-default" onclick="gotoPage(${examPageBeanByCondition.nextPage})">下一页&nbsp;&gt;</button>
				<span class='bottom-txt' style=''>共${examPageBeanByCondition.totalPage}页  共${examPageBeanByCondition.totalRecord}条记录</span>
				<span class="bottom-txt">到第<input type="text" value="${examPageBeanByCondition.currentPage}" id="currentPage" style="width: 40px; height: 35px; text-align: center;">页</span>
...
<script type="text/javascript">
	...
	function gotoPage(currentPage){
		...
		window.location.href="/OA/exam/findExamByCondition?currentPage=" + currentPage;
	}
</script>